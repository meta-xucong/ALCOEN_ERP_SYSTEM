...
class User(db.Model):
    ...
    def can_access_department(self, dept_name: str) -> bool:
        """是否可以访问指定部门的数据"""
        # 超级管理员、总经理、物流经理、总经理助理可以访问所有部门
        if self.is_superadmin or self.role.code in ['general_manager', 'logistics_manager', 'gm_assistant']:
            return True
        # 部门角色只能访问本部门
    ...
    def can_view_contract(self, contract) -> bool:
        ...
        if self.is_superadmin:
            return True
        # 总经理、物流经理、总经理助理可以查看所有
        if self.role.code in ['general_manager', 'logistics_manager', 'gm_assistant']:
            return True
        # 部门PM可以查看本部门所有合同
    ...
    def can_edit_contract(self, contract=None) -> bool:
        ...
        if self.role.code == 'general_manager':
            return True
        # 总经理助理可以编辑所有合同
        if self.role.code == 'gm_assistant':
            return True
        # 部门PM可以编辑本部门所有合同
        if self.role.code == 'department_pm':
